...
## Fallback and alias locales in Wagtail

While many locales serve their own pages as you might expect (`/en-US/somepage/` tries to serve the page with the `somepage` slug in the `en-US` locale), we also have a concept of alias and fallback locales. Some locales may not have their own translated text, so they may serve content in a variation of the language.

For example, the `es-CL` locale may not have a particular page (let's say a `somepage`) in Wagtail, but it can serve the `es-MX` `somepage` instead at the `/es-CL/somepage/` URL. In this case, `es-CL` is considered the alias locale, and `es-MX` is considered the fallback locale.

Note: alias and fallback locales are configured in the code, but the relationship can be seen on the list of locales in Wagtail. Go to `Settings > Locales` and look for the marker just to the right of the locale name. It should look like: `Spanish (Chile) alias → es-MX`.
...
!!! note
    The L10N team is not automatically notified about the new job in Smartling, so it is vital to let them know about the job you created, what it's for and any turnaround timeline needed.
    Every live/published CMS page has a public URL that must be shared with the L10N team in #www-firefox-l10n or #www-l10n (for www.mozilla.org). If the page is still draft, you must generate a draftsharing link instead and provide that to the team.
