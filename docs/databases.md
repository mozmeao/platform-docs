...
* Local builds, by default, use Sqlite, regardless of whether you're working using Docker or on 'bare metal' - this makes it particularly easy to get a copy of Dev, Stage or Prod data to work on things locally.
...
The DB will contain a table that knows the relative paths of the images uploaded to the CMS, but not the actual images. Those are in a cloud storage bucket. To get the images if you need them, see [this documentation](cms.md/#ive-downloaded-a-fresh-db-and-the-images-are-missing)
