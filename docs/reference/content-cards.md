...
!!! note
    ...
    Alternately you could wrap the section of the template using cards to be optional in an `{% if page_content_cards %}` statement, and that way it will not show the section at all if the dict is empty if there are no cards for that page and locale combination.
