...
Once you decide where it should go you can add your redirect. To do this, you add a call to the `bedrock.redirects.util.redirect` helper function in a list named `redirectpatterns` in `redirects.py`. For example:
...
You pass it a regex to match, the destination URL (substitutions from the original URL do work) if the regex matches, and another destination URL if the regex does not match. The match is not case sensitive unless you add the optional `case_sensitive=True` argument.
...
A suite of tests exists for redirects, which is intended as a reference of the redirects we expect to work on the site. This will become a base for implementing these redirects in the app and allow us to test them before release.

### Installation

First follow the [installation instructions](../getting-started/install.md), which will guide you through installing pip and setting up a virtual environment for the tests. The additional requirements can then be installed by using the following commands:

=== "Bedrock"

    ``` bash
    source venv/bin/activate bedrock
    ```

=== "Springfield"

    ``` bash
    source venv/bin/activate springfield
    ```
...
Running the full set of tests requires a deployed instance of the site. Direct the tests to
the site with the `--base-url` command line option:
...
=== "Springfield"
    ...
    ``` bash
    pytest --base-url https://www.firefox.com tests/redirects/
    ```
