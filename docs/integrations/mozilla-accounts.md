...
!!! note
    See the attribution docs ([Mozilla accounts attribution](../measurement/mozilla-accounts.md)) for more a detailed description of the analytics functions these helpers provide.
...
Since Firefox 80 the accounts link and email form macros use [UITour](../integrations/uitour.md) to show the Mozilla accounts page and log the browser into Sync or an account. For non-Firefox browsers or if UITour is not available, the flow uses normal links that allow users to log into the Mozilla accounts website only, without connecting the Firefox Desktop client. This transition was introduced to later migrate Firefox Desktop to an OAuth based client authentication flow.
